...
function sqlServerMiddleware(req, res, next) {
  ...
  // GET /api/db/rooms
  if (req.url === '/api/db/rooms' && req.method === 'GET') {
    (async () => {
      try {
        const data = await registry.guard('sql', async () => {
          const pool = await getPool();
        ...
        console.error('[SQL Server] Rooms query failed:', err.message);
        sendJson(500, { success: false, error: err.message });
      }
    })();
    return;
  ...
  // GET /api/db/rooms/:roomName/assets
  const roomAssetsMatch = req.url?.match(/^\/api\/db\/rooms\/([^/?]+)\/assets/);
  if (roomAssetsMatch && req.method === 'GET') {
    const roomName = decodeURIComponent(roomAssetsMatch[1]);
    const url = new URL(req.url, `http://${req.headers.host || 'localhost'}`);
    const owner = String(url.searchParams.get('owner') || '').trim().toUpperCase();

    (async () => {
      if (!roomName.trim()) {
        sendJson(400, { success: false, error: 'Room name tidak boleh kosong' });
        return;
      }
      ...
      try {
        const data = await registry.guard('sql', async () => {
          const pool = await getPool();
          const result = await pool.request()
            .input('roomName', sql.NVarChar, roomName.trim())
            .query(`
              SELECT
                HIST_ID, BARCODE_ASSET, NAMA_ASSET, CN_ASSET, KODE_KATEGORI_ASSET, KODE_TYPE_ASSET, LOCATION_CODE,
                NAMA_RUANGAN, PIC_RUANGAN, NAMA_KONDISI, KETERANGAN, KETERANGAN_OPNAME, TRANS_DATE, BULAN, TAHUN,
                CREATE_USER, CREATE_DATE, NO_PO, SITE_ID, RUANGAN_ID, KONDISI_BARANG_ID
              FROM (
                SELECT *,
                  ROW_NUMBER() OVER (
                    PARTITION BY BARCODE_ASSET
                ...
                FROM [dbo].[V_REPORT_ALL_DETAIL]
                WHERE BARCODE_ASSET IS NOT NULL AND LTRIM(RTRIM(BARCODE_ASSET)) <> ''
                  AND LTRIM(RTRIM(NAMA_RUANGAN)) = @roomName
              ) sub
              WHERE rn = 1
              ORDER BY BARCODE_ASSET
            `);
        ...
        console.error('[SQL Server] Room assets query failed:', err.message);
        sendJson(500, { success: false, error: err.message });
      }
    })();
    return;
